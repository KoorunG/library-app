...
class BookService(
    ...
    @Transactional
    fun loanBookQueryDsl(request: BookLoanRequest) {
        val book = bookRepository.findByName(request.bookName) ?: fail("해당 이름의 책이 존재하지 않습니다.")
        val user = userRepository.findByName(request.userName) ?: fail("해당 이름의 유저가 존재하지 않습니다.")
    ...
    @Transactional(readOnly = true)
    fun countLoanedBookQueryDsl () = userLoanHistoryQueryRepository.count(UserLoanStatus.LOANED)
    ...
    @Transactional(readOnly = true)
    fun getBookStatisticsByQueryDsl() =
        bookQueryRepository.groupByTypeQueryDsl()
}